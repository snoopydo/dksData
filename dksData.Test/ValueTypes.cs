...
namespace TestProject1
{
	...
	public class ValueTypes
	{
		...
		{
			var results = db.Query<DateTime>("select cast('1 Jan 2011' as datetime) union all select '2 Feb 2012 13:45' union all select '3 Mar 2013' union all select '4 Apr 2014'");

			results.IsSequenceEqualTo(new[] { new DateTime(2011, 1, 1), new DateTime(2012, 2, 2, 13, 45, 0), new DateTime(2013, 3, 3), new DateTime(2014, 4, 4) });
		}
		...
		[TestMethod]
		public void Query_Byte_To_Ints()
		{
			var results = db.Query<int>(@"
									select cast(1 as tinyint)
									union all
									select cast(2 as tinyint)
									union all
									select cast(3 as tinyint)
									union all
									select cast(4 as tinyint)
									union all
									select cast(5 as tinyint)
								");

			results.IsSequenceEqualTo(new[] { 1, 2, 3, 4, 5 });
		}
		#endregion
